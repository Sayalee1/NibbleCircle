...
<form method="get" action="/search2.php" id="search">
  <input name="q" type="text" size="40" placeholder="eg tomato onion potato" />
</form>
...
$searchtag = $searchtag . "'$search', ";

}

$Ingredientsname = substr($searchtag, 0, -2) ;
echo $Ingredientsname;
echo $searchid [1];

$i =strlen($Ingredientsname)==0?0:COUNT(explode(',',$Ingredientsname));


for ($i; $i >0 ;$i--)
{

		$query = "SELECT * FROM recipe where recipe_id in (SELECT recipe_id FROM ingredients where name in ($Ingredientsname) group by recipe_id having count(*)=$i)";
...
echo "<div id = 'contentdesc'>Match found: ".$i."\n".$string."</div>";
